...
> Complete: **Phase 6.3 — one attempt vs. reflection, measured.** Same 12 pinned examples, same model, prompt, and code revision; only the attempt cap changed (1 vs 3). All-four-gates pass: 11/12 with one attempt, 9/12 with reflection; graph passed 9/12 vs 8/12. The gap is not reflection: four first drafts across the two runs failed to parse (`notes` returned as a string) and never reached validation or repair. Among drafts that parsed, 100% passed all static gates in both arms; reflection fired on two files and rescued one for +2 actor calls and +11k tokens. The number that replaces "perfect": **9 to 11 of 12 files pass all static checks per run; 1 to 3 of 12 first drafts fail to parse.** See the [plain-English walkthrough](docs/reflection-ab.md), the [detailed report with the LangSmith links and screenshot](docs/phase-6.3-report.md), and the [6.2 baseline](docs/phase-6.2-report.md). All 95 offline tests pass. **Next: 6.4 LLM-as-judge**, with the parse failure logged as the top converter gap. [Session restart notes](docs/session-handoff.md).
>
