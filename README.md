...
> Complete: **Phase 6.1 — evaluation dataset**. Twelve conversion examples across six scenarios are uploaded and verified in LangSmith; all eight Selenium and eight Playwright fixture tests pass. See the [completion report](docs/phase-6.1-report.md), [fixture walkthrough](docs/evaluation-fixtures.md), and [upload theory and code](docs/evaluation-upload.md). **Phase 6.2 is in progress:** the [target adapter](docs/evaluation-target.md), [deterministic evaluators](docs/evaluation-evaluators.md), and [experiment runner](docs/evaluation-runner.md) are implemented. The runner pins inputs/configuration, saves detailed reports, and verifies cloud evidence; 80 offline tests pass. The first live scored LangSmith experiment is next. [Session restart notes](docs/session-handoff.md).
>
