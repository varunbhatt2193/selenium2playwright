...
> Complete: **Phase 6.5 — does the repair loop earn its extra calls?** The 6.3 A/B (one attempt vs up to three, only the attempt cap changes) was repeated per actor with the same Opus critic. With **Haiku** writing: static gates 9/12 → 12/12, fully passed reports 2/12 → 9/12, 8 of 12 rows repaired, every compile failure fixed, at ×1.9 cost. With **Opus** writing: 12/12 static either way, 6/12 → 11/12 fully passed, only 2 repairs. Reflection matters when the first draft is often wrong; Opus rarely needs it, and Opus with the loop is still the cheapest route to the best result. The **Sonnet** reflection arm hit an Anthropic credit failure mid-run and is refused (gap T10); its one-attempt arm (12/12 static, 9/12 passed) is valid and the rerun is queued. Walkthroughs: [Haiku](docs/reflection-haiku-ab.md), [Opus](docs/reflection-ab.md); reports: [Haiku](docs/phase-6.5-haiku-report.md), [Sonnet](docs/phase-6.5-sonnet-report.md), [Opus](docs/phase-6.3-report.md). All 109 offline tests pass. **Next: Sonnet rerun, then 6.4 LLM-as-judge.** [Session restart notes](docs/session-handoff.md).
>
> ![One attempt vs reflection per actor, same Opus critic](docs/reflection-shootout.svg)
>
> Every bar is copied from a verified LangSmith receipt by `scripts/render_actor_shootout.py`; the numbers behind it are in [reflection-shootout-table.md](docs/reflection-shootout-table.md).
>
